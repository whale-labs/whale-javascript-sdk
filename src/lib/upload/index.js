...
function sendPost(data, callback) {
  if (
    Util.GetUrlParam("visual") &&
    Util.GetUrlParam("visual").indexOf("true") > -1
  ) {
    return;
  }
  if (sendType !== "img") {
  ...
  var successCheckPost = function (data) {
    ...
    for (var i = 0; i < postDataList.length; i++) {
      ...
      for (var y = 0; y < data.length; y++) {
        if (
          data[y].xwhen === postDataList[i].xwhen ||
          !data[y].xwhen ||
          !postDataList[i].xwhen
        ) {
          delData = true;
        }
      }
      if (
        delData === false &&
        Util.paramType(postDataList[i]) === "Object" &&
        !Util.isEmptyObject(postDataList[i])
      ) {
  ...
  if (lifecycle.upload && lifecycle.upload.init) {
    ...
    Util.paramType(window.AnalysysModule) === "Object" &&
    Util.paramType(window.AnalysysModule.uploadData) === "Function"
  ) {
...
function checkLogBaseJson(obj) {
  if (Util.paramType(obj) === "Object" && !Util.isEmptyObject(obj)) {
    ...
    for (var i = 0; i < baseConfig.baseJson.length; i++) {
      ...
      if (
        !obj[key] ||
        (Util.paramType(obj[key]) === "Object" && Util.isEmptyObject(obj[key]))
      ) {
        status = false;
      }
    }
...
function checkSavaData(list) {
  ...
  if (
    Util.paramType(list) === "Object" &&
    !Util.isEmptyObject(list) &&
