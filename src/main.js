...
function _createAnsSDK() {
  if (Util.paramType(window.AnalysysAgent) === "Array") {
    ...
    if (window.navigator.userAgent.indexOf("AnalysysAgent/Hybrid") > -1) {
      if (
        lifecycle.AnalysysAgent &&
        Util.paramType(lifecycle.AnalysysAgent.hybridAns) === "Object"
      ) {
      ...
      for (var y = 0; y < AnalysysAgent.length; y++) {
        ...
        if (
          Util.objHasKay(ans, yItem[0]) &&
          (yItem[0] === "identify" ||
            yItem[0] === "alias" ||
            yItem[0].indexOf("Super") > -1)
        ) {
      ...
      try {
        ...
        if (Util.paramType(AnalysysAgent) === "Array") {
          for (var z = 0; z < AnalysysAgent.length; z++) {
            ...
            if (
              Util.objHasKay(ans, zItem[0]) &&
              zItem[0] !== "identify" &&
              zItem[0] !== "alias" &&
              zItem[0].indexOf("Super") < 0
            ) {
